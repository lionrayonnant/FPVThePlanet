...
func (ctx context) findPlace(lat, lon float64) (fly.Trigger, error) {
	...
	for _, v := range ctx.AltitudeManifest.Triggers {
		// dist is in degrees, but v.Radius is in meters (trigger's "radius" xml
		// attribute). Convert dist to meters (rough, non-spherical approximation)
		// so it's comparable to v.Radius, otherwise nearly every trigger's radius
		// check trivially passes and the wrong (nearest-by-degree, not
		// actually-containing) region gets picked.
		distDeg := math.Sqrt(math.Pow(lat-v.Lat, 2) + math.Pow(lon-v.Lon, 2))
		dist := distDeg / 180 * math.Pi * mth.EarthRadiusByLatitude(lat)
