...
import {
	...
	VOICES, PERCUSSIVE, CULMINATION_SCORES, scoreFor,
...
// --- partitions de culmination (#101) ---------------------------------------

const VARIANT_MS = [1000, 2000, 3000, 4000]; // V1..V4, cf. tools/culmination-model.mjs

t('CULMINATION_SCORES : une partition par famille de HACK_TYPES, et rien d\'autre', () => {
	assert.deepEqual(Object.keys(CULMINATION_SCORES).sort(), [...HACK_TYPES].sort());
});

t('CULMINATION_SCORES : chaque événement nomme une voix connue', () => {
	for (const [family, score] of Object.entries(CULMINATION_SCORES)) {
...
t('CULMINATION_SCORES : temps normalisé croissant, dans [0,1]', () => {
	for (const [family, score] of Object.entries(CULMINATION_SCORES)) {
...
t('CULMINATION_SCORES : les six sont réellement distinctes', () => {
	...
	// de voix.
	const shapes = Object.entries(CULMINATION_SCORES)
...
t('CULMINATION_SCORES : montée, détonation en couches, puis des shrapnels dispersés (§36)', () => {
	for (const [family, score] of Object.entries(CULMINATION_SCORES)) {
...
t('scoreFor : transporte le pan des shrapnels', () => {
	for (const family of HACK_TYPES) {
		const raw = CULMINATION_SCORES[family];
